...
const fetchUrl = async (url: string, signal?: AbortSignal): Promise<ArrayBuffer> => {
	...
	if (!res.ok) throw new Error(`bot-core: telegram 下载失败 http ${res.status}`)
	...
	const addAttachment = (part: Attachment<'telegram'>['part']) => {
		...
		attachments.push({
			...
			fetch: async (signal) => {
				...
				if (!fileId) throw new Error('bot-core: telegram 缺少 fileId 和 downloadUrl')
				const info = await resolveTelegramFile(ctx, fileId)
				if (!info.downloadUrl) throw new Error('bot-core: telegram 缺少 downloadUrl')
