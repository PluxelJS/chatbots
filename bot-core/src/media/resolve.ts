...
const fetchUrl = async (url: string, signal?: AbortSignal): Promise<Buffer> => {
	...
	if (!res.ok) throw new Error(`bot-core: 下载媒体失败 ${res.status} ${res.statusText}`)
...
const avatarToMedia = (
	...
	return {
		...
		key: key ?? undefined,
		fetch: async (signal) => {
			if (signal?.aborted) throw new Error('bot-core: resolveMedia aborted')
			return img.data
		},
	}
}
...
export const resolveMedia = async (items: MediaItem[], opts?: { concurrency?: number; signal?: AbortSignal }): Promise<ResolvedMediaItem[]> => {
	...
	const run = async () => {
		while (true) {
			...
			if (!data) throw new Error('bot-core: media 无可用下载方式')
