...
/** Reply payload: single message or explicit multi-message batch. */
export type ReplyPayload = MessageContent | MessageBatch
...
export type PolicyForPlatform<P extends Platform> =
	P extends keyof BotCorePlatformPolicyRegistry ? BotCorePlatformPolicyRegistry[P] : AdapterPolicy
...
export type Message<P extends Platform = Platform> = {
	 ...
	 *
	 * 行为说明：
	 * - 支持 `Part[]` 或 `MessageBatch`（推荐用 `parts\`...\`` / `mc.*` 构建；多条消息用 `mc.batch(...)`）
	 * - 允许混合内容（多段文本、多张图片、文件夹杂文本等）
	 * - 可能会拆成多条消息发送（按输入顺序，拆分规则见 bot-core DESIGN.md）
	 ...
	 */
	reply: (payload: ReplyPayload, options?: ReplyOptions) => Promise<void>
	/**
	 * 显式发送纯文本（原子能力）。
	 *
