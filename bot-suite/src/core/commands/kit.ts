...
import { CommandError } from 'pluxel-plugin-bot-core'
import type { KvRateRule, RateDecision, RateRule, Rates } from 'pluxel-plugin-kv'

declare module 'pluxel-plugin-bot-core' {
...
export type CommandKit<C = ChatbotsCommandContext> = {
	...
	/** Group commands under a namespace (callback receives the same augmented kit). */
...
export function withPermissions<C extends ChatbotsCommandContext>(
	...
	const wrapped: CommandKit<C> = {
		...
		group(name, def) {
			return kit.group(name, () => def(wrapped))
		},
...
export function createPermissionCommandKit<C extends ChatbotsCommandContext>(
	...
	if (opts?.rates) {
		plugins.push(createRatesPlugin(opts.rates, { scopeKey: opts.scopeKey ?? 'bot-suite' }))
	}
