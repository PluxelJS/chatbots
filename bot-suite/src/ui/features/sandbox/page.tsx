...
import type { Part, Platform } from 'pluxel-plugin-bot-core'
import {
	PartsMessage,
	PartsShowcasePanel,
	quickReplies,
...
const isKnownPlatform = (value: string): value is Platform =>
	value === 'sandbox' || value === 'kook' || value === 'telegram'
...
const sanitizeSession = (input: any): SandboxSession | null => {
	...
	return {
		...
		platform: typeof input.platform === 'string' && isKnownPlatform(input.platform) ? input.platform : DEFAULT_PLATFORM,
			...
			if (list.length) {
				...
				const active =
					typeof parsed?.activeSessionId === 'string' && list.some((s) => s.id === parsed.activeSessionId)
						...
						: list[0]!.id
				setActiveId(active)
				seq.current = Math.max(1, ...list.map((session) => extractSessionSeq(session.id)))
			}
		} catch {
			// ignore
		} finally {
				...
				<Stack gap="sm">
					...
					<Divider />
						...
						<Select
							...
							onChange={(v) => {
								if (v && isKnownPlatform(v)) session.update({ platform: v })
							}}
						/>
