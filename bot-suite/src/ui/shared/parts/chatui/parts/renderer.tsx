...
import type {
	...
	MentionPart,
	Part,
} from 'pluxel-plugin-bot-core/web'

const toBlobBytes = (data: Uint8Array | ArrayBufferLike): Uint8Array<ArrayBuffer> => {
...
const isInlinePart = (part: Part): part is InlinePart => {
	return part.type === 'text' || part.type === 'mention' || part.type === 'link' || part.type === 'styled'
}

const isTextLike = (part: Part): part is InlinePart | CodeBlockPart => {
	return part.type === 'codeblock' || isInlinePart(part)
}
...
const useBlobUrl = (data?: Uint8Array | ArrayBufferLike, mime?: string) => {
	const blobUrl = useMemo(() => {
		...
		const blob = new Blob([toBlobBytes(data)], { type: mime ?? 'application/octet-stream' })
	...
	const items = useMemo(() => {
		if (typeof URL === 'undefined' || typeof Blob === 'undefined') {
			return parts.flatMap((item, index) =>
				item.url
					? [{ src: item.url, id: item.fileId ?? `img-${index}`, revoke: null as string | null }]
					: [],
			)
		}

		return parts.flatMap((item, index) => {
			const id = item.fileId ?? `img-${index}`
			if (item.url) return [{ src: item.url, id, revoke: null as string | null }]
			if (!item.data) return []
			const blob = new Blob([toBlobBytes(item.data)], { type: item.mime ?? 'application/octet-stream' })
			const src = URL.createObjectURL(blob)
			return [{ src, id, revoke: src }]
		})
	}, [parts])
...
function FilePartBlock({ part, showMeta }: { part: FilePart; showMeta: boolean }) {
	...
	const file = useMemo(() => {
		...
		const blob = part.data
			? new Blob([toBlobBytes(part.data)], { type: part.mime ?? 'application/octet-stream' })
...
const renderCardBlocks = (
	...
	parts.forEach((part) => {
		...
		if (part.type === 'audio' || part.type === 'video') {
			flushImages()
			flushInline()
			const fileLike: FilePart = {
				type: 'file',
				name: part.name ?? (part.type === 'audio' ? 'audio' : 'video'),
				mime: part.mime,
				url: part.url,
				data: part.data,
				size: part.size,
		...
		flushImages()
		if (isInlinePart(part)) inlineBuffer.push(part)
	})
...
export function PartsMessage({
	...
	const hasMedia = parts.some((part) => part.type === 'image' || part.type === 'file' || part.type === 'audio' || part.type === 'video')
	if (!hasMedia) {
