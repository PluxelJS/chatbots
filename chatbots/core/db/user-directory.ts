...
export class UserDirectory {
	...
	async ensureUserForMessage(msg: AnyMessage): Promise<{ user: UnifiedUser; identity: UnifiedIdentity }> {
		...
		const promise = this.ensureUserForIdentityUncached(identity).finally(() => {
	...
	private async ensureUserForIdentityUncached(
		...
		if (existing) {
			...
			if (!userRow) {
				...
				const created = await this.createUserWithIdentity(em, identity)
				return created
			}
			const full = await this.getUserById(em, userRow.id)
	...
	private async createUserWithIdentity(
		...
		try {
			return await em.transactional(async (tx) => {
				const user = { createdAt: now } as Omit<UnifiedUserRow, 'id'>
				...
				const userFull = await this.getUserById(tx, userId)
			...
			if (fallback) {
				const userFull = await this.getUserById(em, fallback.userId)
				return { user: userFull, identity }
			}
			throw e
		}
	...
	private async getUserById(em: EntityManager, userId: number): Promise<UnifiedUser> {
		const userRow = (await em.findOne<UnifiedUserRow>(
			this.entities.user.entityName as any,
			{ id: userId },
		)) as UnifiedUserRow | null
