...
import type { SandboxEvent, SandboxMessage, SandboxPart, SandboxSnapshot } from './sandbox-types'
...
const serializeParts = (parts: Part[]): SandboxPart[] =>
	parts.map((part) => {
		if ((part.type === 'image' || part.type === 'file') && part.data) {
			return { ...part, data: encodeBinary(part.data as ArrayBufferLike | ArrayBufferView) }
		}
		return part
	})
