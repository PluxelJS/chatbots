...
This folder contains the Chatbots plugin UI pages registered by `chatbots` (see `chatbots/chatbots/ui/index.tsx`).

## Permissions Console

Entry: `chatbots/chatbots/ui/features/permissions/page.tsx`

### Roles (权限组)

- A “权限组” is a `Role` in the permissions system.
- Roles are displayed **sorted by `rank` descending**, then `roleId` ascending (see `chatbots/chatbots/ui/features/permissions/hooks.ts`).
...
The permissions console uses the `chatbots` RPC extension (implemented in `chatbots/chatbots/src/core/sandbox`).
