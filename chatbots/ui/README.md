...
Entry: `chatbots/chatbots/ui/permissions.tsx`

### Roles (权限组)

- A “权限组” is a `Role` in the permissions system.
- Roles are displayed **sorted by `rank` descending**, then `roleId` ascending (see `chatbots/chatbots/ui/hooks.ts`).
...
The permissions console uses the `chatbots` RPC extension (implemented in `chatbots/chatbots/core/sandbox.ts`).
